...
- **Hearts (❤):** your lives, up to 3. Every run starts with at least one. Buy more in the shop, pick one up (they only appear while you have none left), or beat a boss to refill all three. A spent heart (solo or co-op) throws you back up with a HARD SHIELD. With ads off (`ADS_ENABLED` in `js/config.js`) there is no revive offer: out of hearts ends the run.
